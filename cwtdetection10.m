...
while(i<min(length(st),length(en)))
    ...
    [maxpos,posloc] = max(tmparray(tpatt(2):tpatt(end)));
    
    Ppeak = [Ppeak,posloc(end)+st(i)+tpatt(2)-2];
    Pbeginarray = coefvp(st(i)+tpatt(2)-1:posloc(end)+st(i)+tpatt(2)-2);
    [Pinvalue,Pbeginloc] = min(Pbeginarray);
    Pbegin = [Pbegin, Pbeginloc(end)+st(i)+tpatt(2)-2];
    Pendarray = coefvp(posloc(end)+st(i)-1:st(i)+tpatt(end)-1);
    [Pendvalue,Pendloc] = min(Pendarray);
    Pend = [Pend, Pendloc(end)+posloc(end)+st(i)-2];
    ...
    clear tmparray tmparraypos tmparrayneg posloc negpoc Pbeginarray Pendarray
end
